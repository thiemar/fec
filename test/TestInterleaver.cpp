...
TEST(InterleaverTest, NonPuncturing) {
    ...
    std::array<Thiemar::bool_vec_t, TestInterleaver::in_buf_len()> in = {};
    std::array<uint8_t, TestInterleaver::out_buf_len()> out;
    in[0] = 0xFFFFFFFF00000000u;
    in[1] = 0xFFFFFFFF00000000u;

    out = TestInterleaver::interleave(in);
...
TEST(DeinterleaverTest, NonPuncturing) {
    ...
    std::array<Thiemar::bool_vec_t, TestInterleaver::in_buf_len()> in = {};
    std::array<Thiemar::bool_vec_t, TestInterleaver::in_buf_len()> in_reconstructed = {};
    std::array<uint8_t, TestInterleaver::out_buf_len()> out;
    in[0] = 0x581A3C6F00000000u;
    in[1] = 0x9C7BE54200000000u;

    out = TestInterleaver::interleave(in);
    in_reconstructed = TestInterleaver::deinterleave(out);
